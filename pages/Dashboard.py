...
total_sales_by_ID = dfProd.groupby("ProdName")["price"].sum()
...
# Create a line chart
fig1 = px.line(total_sales_by_date, x='Date', y='TotalAmount', title='Total Sales by Date')
# Display the chart using Streamlit
st.plotly_chart(fig1)

# Visualization: Quantity Sold per Product
st.header('Quantity Sold per Product')
quantity_per_product = dfProd.groupby('ProdName')['Quantity'].sum().reset_index()
fig2 = px.scatter(quantity_per_product, x='ProdName', y='Quantity', size='Quantity', title='Quantity Sold per Product')
st.plotly_chart(fig2)

# Create a pie chart
fig_pie = px.pie(quantity_per_product, values='Quantity', names='ProdName', title='Quantity Sold per Product (Pie Chart)')
st.plotly_chart(fig_pie)

earning_per_product = dfProd.groupby('ProdName')['price'].sum().reset_index()
fig_pie = px.pie(earning_per_product, values='price', names='ProdName', title='Earning per Product (Pie Chart)')
st.plotly_chart(fig_pie)
